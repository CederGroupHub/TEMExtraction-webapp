...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', required=False,
                        ...
                        help="Path to image if inference is to be run on single image")
    ...
    if args.command == "train":
    ...
    elif args.command == 'inferone':
        path_image = args.image_path
        original_image = np.array(Image.open(path_image))
        results = model.detect([original_image], verbose=1)
        r = results[0]
        f = 'image'
        ...
        masks, class_ids = visualize.display_instances(original_image, r['rois'], r['masks'], r['class_ids'],
                                    class_names, r['scores'], index=0, filename=f)
